...
var getLink = "http://localhost:7200/repositories/PRC_Cinema" + "?query=" 


Filmes.getLista = async function(){
    var query = `select ?f ?id ?titulo ?duracao ?data ?lingua ?pop ?res where {
        ?f a c:Filme .
        ?f c:título ?titulo .
        ?f c:duração ?duracao .
...
Filmes.getGenerosDoFilme = async function(idFilme){
    var query = `select ?g ?gnome ?idGenero where {
